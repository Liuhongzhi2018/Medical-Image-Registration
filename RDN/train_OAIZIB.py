...
def evaluate_OAIZIB(args, model, steps, Logging, type):
    ...
    Dice, Jacc, Jacb, Time = [], [], [], []
    eval_dataset = datasets.OAIZIBTest(args)
    # if args.local_rank == 0:
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % args.val_freq == 0:
                # if args.local_rank == 0:
                model.eval()
                evaluate_OAIZIB(args, model, total_steps + 1, Logging, type="teacher")
...
if __name__ == '__main__':
    ...
    if args.dataset == "ACDC" or "OASIS" or "OAIZIB":
