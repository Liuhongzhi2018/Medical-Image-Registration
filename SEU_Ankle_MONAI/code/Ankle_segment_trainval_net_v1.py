...
# CUDA_VISIBLE_DEVICES=1 python Ankle_segment_trainval_net.py train --data_folder "./data/Task12_121_55" --model_folder "./segment_checkpoints/ankleseg_12155_UNet_bs1res2ep500_0422"
# CUDA_VISIBLE_DEVICES=1 python Ankle_segment_trainval_net_v1.py infer --data_folder "./data/Task12_121_55" --model_folder "./segment_checkpoints/ankleseg_Task12_121_55_UNETR_ep500_0403"
...
def train(data_folder=".", model_folder="runs"):
    ...
    train_images = sorted(glob.glob(os.path.join(data_folder, "imagesTr", "*.nii.gz")))
    train_labels = sorted(glob.glob(os.path.join(data_folder, "labelsTr", "*.nii.gz")))
    test_images = sorted(glob.glob(os.path.join(data_folder, "imagesTs", "*.nii.gz")))
    test_labels = sorted(glob.glob(os.path.join(data_folder, "labelsTs", "*.nii.gz")))
    ...
    train_ds = monai.data.CacheDataset(data=train_files, transform=train_transforms, cache_rate=0.5) # , cache_rate=0.1)
    ...
    val_ds = monai.data.CacheDataset(data=val_files, transform=val_transforms, cache_rate=0.) #, cache_rate=0.1)
    ...
    # )
    val_post_transform = monai.transforms.Compose(
        [AsDiscreted(keys=("pred", "label"), argmax=(True, False), to_onehot=3)]
    )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        # stream=sys.stdout, 
        level=logging.INFO,
        filename=os.path.join(args.model_folder, "train.log"),
        filemode='a')
    # random.seed(0)
