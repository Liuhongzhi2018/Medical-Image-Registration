...
def similarity_loss(img1, img2_warped):
    ...
    # print(f"similarity_loss img1 shape: {img1.shape} sizes: {sizes}")
    # similarity_loss img1 shape: torch.Size([1, 1, 80, 80, 80]) sizes: 512000
    
    # flatten1 = img1.view(-1, sizes)
    flatten1 = img1.contiguous().view(-1, sizes)
    # flatten2 = img2_warped.view(-1, sizes)
    flatten2 = img2_warped.contiguous().view(-1, sizes)
