...
import model.networks_DiffuseMorph as networks_DiffuseMorph

class DiffuseMorph_DDPM(BaseModel):
    def __init__(self, opt):
        super(DiffuseMorph_DDPM, self).__init__(opt)
        # define network and load pretrained models
        self.netG = self.set_device(networks_DiffuseMorph.define_G(opt))
    ...
    def test_registration(self, continuous=False):
        ...
        # print(f"self.data['M'] shape: {self.data['M'].shape}")
        # self.data['M'] shape: torch.Size([1, 1, 32, 128, 128])
        nsample = self.data['M'].shape[2]
        if isinstance(self.netG, nn.DataParallel):
        ...
        self.netG.train()
        # print(f"out_M {self.out_M.shape} flow {self.flow.shape} contD {self.contD.shape} contF {self.contF.shape}")
        # out_M torch.Size([1, 1, 32, 128, 128]) flow torch.Size([1, 3, 32, 128, 128]) contD torch.Size([1, 32, 128, 128]) contF torch.Size([3, 32, 128, 128])
