...
# third-party modules
import statistics
from scipy.ndimage import _ni_support, map_coordinates
...
if __name__ == "__main__":
    ...
    diffusion = Model.create_DiffuseMorph_model(opt)
    ...
    while current_epoch < n_epoch:
        ...
        if current_epoch % opt['train']['val_freq'] == 0:
            ...
            for istep, test_data in enumerate(test_loader):
                ...
                defm_frames_visual = visuals['contD'].numpy().transpose(0, 2, 3, 1)
                # flow_frames = visuals['contF'].numpy().transpose(0, 3, 4, 2, 1)
                flow_frames = visuals['contF'].numpy().transpose(2, 3, 1, 0)
                # flow_frames_ES = flow_frames[-1]
                flow_frames_ES = flow_frames
