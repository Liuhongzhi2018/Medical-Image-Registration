...
def compute_Dice_HD95(pre_path, gt_path, out_path):

    pre_file_list = os.listdir(pre_path)
    gt_file_list = os.listdir(gt_path)
    sum_Dice = sum_Dice1 = sum_Dice2 = 0
    sum_HD95 = sum_HD951 = sum_HD952 = 0
    list_Dice = []
    list_HD95 = []
    count = 1
    # print(f"pre_file_list: {pre_file_list} {len(pre_file_list)}")
    # print(f"gt_file_list: {gt_file_list} {len(gt_file_list)}"

    with open(out_path, 'w', newline='') as fi:
        writer = csv.writer(fi)
        writer.writerow(['image_id', 'Dice_1', 'Dice_2', 'HD95_1', 'HD95_2', 'mean_Dice', 'mean_HD95'])
    
        for f in pre_file_list:
            pred_file = os.path.join(pre_path, f)
            gt_file = os.path.join(gt_path, f)
            print(f"N: {count} pred file: {pred_file} gt_file: {gt_file}")

            gt_sitk = sitk.ReadImage(gt_file)
            gt_data = sitk.GetArrayFromImage(gt_sitk).astype(np.int16)
            pred_sitk = sitk.ReadImage(pred_file)
            pred_data = sitk.GetArrayFromImage(pred_sitk).astype(np.int16)
            print(f"gt_data shape: {gt_data.shape}")
            class_num = np.unique(gt_data)
            # class_list = list(class_num)
            ...
            for c in class_num:
                ...
                ngt_data[gt_data==c] = 1
                npred_data = np.zeros_like(pred_data)
                npred_data[pred_data==c] = 1
            ...
            writer.writerow([f, 
                             ...
                             str(n_hd95_list[0]), 
                             str(n_hd95_list[1]),
                             str(statistics.mean(n_dice_list)),
                             str(statistics.mean(n_hd95_list))])
            print(f"{f} \
                   Dice_1 {n_dice_list[0]} Dice_2 {n_dice_list[1]} \
                   HD95_1 {n_hd95_list[0]} HD95_2 {n_hd95_list[1]} \
                   mean_Dice {statistics.mean(n_dice_list)} mean_HD95 {statistics.mean(n_hd95_list)}")
            
            sum_Dice += statistics.mean(n_dice_list)
            sum_HD95 += statistics.mean(n_hd95_list)
            sum_Dice1 += n_dice_list[0]
            sum_Dice2 += n_dice_list[1]
            sum_HD951 += n_hd95_list[0]
            sum_HD952 += n_hd95_list[1]

            count += 1
            # if count == 5: break

        all_mean_Dice = sum_Dice / len(pre_file_list)
        all_mean_Dice1 = sum_Dice1 / len(pre_file_list)
        all_mean_Dice2 = sum_Dice2 / len(pre_file_list)
        all_mean_HD95 = sum_HD95 / len(pre_file_list)
        all_mean_HD951 = sum_HD951 / len(pre_file_list)
        all_mean_HD952 = sum_HD952 / len(pre_file_list)
        # all_mean_Dice = sum_Dice / 5
        # all_mean_HD95 = sum_HD95 / 5
        writer.writerow(['All', str(all_mean_Dice1), str(all_mean_Dice2), str(all_mean_HD951), str(all_mean_HD952), str(all_mean_Dice), str(all_mean_HD95)])
    print(f"all_mean_Dice: {all_mean_Dice} all_mean_HD95: {all_mean_HD95}")

def compute_P_R_S_S(pre_path, gt_path, out_path):

    pre_file_list = os.listdir(pre_path)
    gt_file_list = os.listdir(gt_path)
    sum_pre = sum_rec = sum_spe = sum_sen = 0
    count = 1
    # print(f"pre_file_list: {pre_file_list} {len(pre_file_list)}")
    # print(f"gt_file_list: {gt_file_list} {len(gt_file_list)}"
    ...
    with open(out_path, 'w', newline='') as fi:
        writer = csv.writer(fi)
        writer.writerow(['image_id', 'precision', 'recall', 'specificity', 'sensitivity'])
        ...
        for f in pre_file_list:
            ...
            pre = precision(pred_data, gt_data)
            rec = recall(pred_data, gt_data)
            spe = specificity(pred_data, gt_data)
            sen = sensitivity(pred_data, gt_data)
            ...
            print(f"{f} \
                   ...
                   specificity {spe} sensitivity {sen}")
        ...
        all_mean_pre = sum_pre / len(pre_file_list)
        all_mean_rec = sum_rec / len(pre_file_list)
        all_mean_spe = sum_spe / len(pre_file_list)
        all_mean_sen = sum_sen / len(pre_file_list)

        writer.writerow(['All', str(all_mean_pre), str(all_mean_rec), str(all_mean_spe), str(all_mean_sen)])
    print(f"all_mean_pre: {all_mean_pre} all_mean_recall: {all_mean_rec} all_mean_spe: {all_mean_spe} all_mean_sen: {all_mean_sen}")
...
if __name__ == "__main__":
    ...
    """
    pre_path = "/mnt/no1/lhz/Github/MONAI/Ankle_fracture_class/segment_checkpoints/ankleseg_12155_nnUNet_0524/predict"
    gt_path = "/mnt/no1/lhz/Github/MONAI/Ankle_fracture_class/data/Task01_121_55/labelsTs"
    out1_path = "/mnt/no1/lhz/Github/MONAI/Ankle_fracture_class/segment_checkpoints/ankleseg_12155_nnUNet_0524/evaluation_Dice_HD95.csv"
    out2_path = "/mnt/no1/lhz/Github/MONAI/Ankle_fracture_class/segment_checkpoints/ankleseg_12155_nnUNet_0524/evaluation_P_R_S_S.csv"

    compute_Dice_HD95(pre_path, gt_path, out1_path)
    compute_P_R_S_S(pre_path, gt_path, out2_path)
