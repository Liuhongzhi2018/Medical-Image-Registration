    ...
    while current_epoch < n_epoch:
        ...
        if current_epoch % opt['train']['save_checkpoint_epoch'] == 0:
            ...
            for istep, test_data in enumerate(test_loader):
                ...
                md, mhd95, d_list, hd95_list = compute_Dice_HD95(regist_seg_int, label_seg_int, ED_spacing)
                md_list.append(md)
                ...
                logger.info(f"mean Dice {md} - {d_list}")
                logger.info(f"mean HD95 {mhd95} - {hd95_list}")
            ...
            logger.info(f"Epoch: {current_epoch} Current Dice {cur_avg_dice} HD95 {cur_avg_hd95} Best_Dice {best_avg_Dice} Best_HD95 {best_avg_HD95} at epoch {best_epoch}")
