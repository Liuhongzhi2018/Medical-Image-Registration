...
if __name__ == "__main__":
    ...
    train_set = Data.create_dataset_3D_resize(dataset_opt, phase)
    ...
    best_epoch, best_avg_Dice, best_avg_HD95 = 0, 0, 10000
    while current_epoch < n_epoch:
        ...
        if current_epoch % opt['train']['save_checkpoint_epoch'] == 0:
            md_list, mhd95_list = [], []
            test_set = Data.create_dataset_3D_resize(dataset_opt=opt['datasets']['test'], phase='test')
            ...
            for istep, test_data in enumerate(test_loader):
                ...
                md, mhd95, d_list, hd95_list = compute_Dice_HD95(regist_seg1, label_seg1)
                md_list.append(md)
                ...
                print(f"------ Saving training samples {istep} {test_data['Name'][0]} ------")
                print(f"mean Dice {md} - {d_list}")
                print(f"mean HD95 {mhd95} - {hd95_list}")
            
            cur_avg_dice = statistics.mean(md_list)
            cur_avg_hd95 = statistics.mean(mhd95_list)
            if cur_avg_dice > best_avg_Dice and cur_avg_hd95 < best_avg_HD95:
                best_epoch = current_epoch
                best_avg_Dice = cur_avg_dice
                best_avg_HD95 = cur_avg_hd95
            print(f"Epoch: {current_epoch} Current Dice {cur_avg_dice} HD95 {cur_avg_hd95} Best_Dice {best_avg_Dice} Best_HD95 {best_avg_HD95} at epoch {best_epoch}")
              
        if current_epoch % opt['train']['save_checkpoint_epoch'] == 0:
            # /mnt/no1/lhz/Github/Registration/SEUFSDiffReg/model/model.py  def save_network(self, epoch, iter_step)
            diffusion.save_network(current_epoch, current_step)
            print(f"------ Saving training checkpoint ------")