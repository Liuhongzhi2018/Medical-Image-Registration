    ...
    while current_epoch < n_epoch:
        ...
        if current_epoch % opt['train']['save_checkpoint_epoch'] == 0:
            ...
            for istep, test_data in enumerate(test_loader):
                ...
                sitk.WriteImage(savedSample_origin, os.path.join(opt['path']['samples'], test_data['Name'][0]+"_EDlabel_ep"+str(current_epoch)+".nii.gz"))
                sitk.WriteImage(savedSample_label, os.path.join(opt['path']['samples'], test_data['Name'][0]+"_ESlabel_ep"+str(current_epoch)+".nii.gz"))
                sitk.WriteImage(savedSample_defm, os.path.join(opt['path']['samples'], test_data['Name'][0]+"_Reg_ep"+str(current_epoch)+".nii.gz"))
                sitk.WriteImage(savedSample_regist, os.path.join(opt['path']['samples'], test_data['Name'][0]+"_Reglabel_ep"+str(current_epoch)+".nii.gz"))
