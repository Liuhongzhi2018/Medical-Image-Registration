...
def register(epoch, name, output, def_out, y_seg, sample_dir):
    ...
    warp_seg = F.interpolate(warp_seg, size=shape_img)
    warp_seg_array = warp_seg.squeeze().detach().cpu().numpy().transpose(2, 1, 0).astype(np.uint8)
    
    seg_gt = F.interpolate(y_seg, size=shape_img)
    ...
    warped_img_path = os.path.join(sample_dir, name.split('.')[0] + '_' + str(epoch) + '_warped_img.nii.gz')
    warped_seg_path = os.path.join(sample_dir, name.split('.')[0] + '_' + str(epoch) + '_warped_seg.nii.gz')
    warped_flow_path = os.path.join(sample_dir, name.split('.')[0] + '_' + str(epoch) + '_warped_deformflow.nii.gz')
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                logger.info(f"{epoch} : {eval_dsc.avg}")
