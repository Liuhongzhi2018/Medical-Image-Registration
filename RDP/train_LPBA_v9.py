...
def save_samples(epoch, name, output, def_out, y_seg, sample_dir):
    ...
    # save_samples warp_img_array: (160, 192, 160) warp_seg_array: (160, 192, 160) deform: (160, 192, 160, 3)
...
def main():
    batch_size = 1
    train_file = '/mnt/lhz/Github/Image_registration/RDP/images/LPBA/train_img_seg_list.txt'
    val_file = '/mnt/lhz/Github/Image_registration/RDP/images/LPBA/test_img_seg_list.txt'
    # checkpoint_dir = '/mnt/lhz/Github/Image_registration/RDP_checkpoints/LPBA/'
    weights = [1, 1]  # loss weights
    lr = 0.0001
    curr_time = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime())
    checkpoint_dir = os.path.join('/mnt/lhz/Github/Image_registration/RDP_checkpoints', 'LPBA_'+curr_time)
    ...
    for epoch in range(epoch_start, max_epoch + 1):
        ...
        for data in train_loader:
            ...
            logger.info('Epoch {} iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Reg: {:.6f}'.format(epoch, idx, len(train_loader), loss.item(), loss_vals[0].item(), loss_vals[1].item()))
        ...
        if epoch % 10 == 0:
            with torch.no_grad():
                for data in val_loader:
                    ...
                    # x_seg shape: torch.Size([1, 1, 64, 64, 64]) y_seg shape: torch.Size([1, 1, 64, 64, 64])
                    # print(f"Val {name} in x_seg: {torch.unique(x_seg)} y_seg: {torch.unique(y_seg)}")

                    output = model(x, y)
                    def_out = reg_model([x_seg.cuda().float(), output[1].cuda()])
                    # print(f"Val {name} output shape: {output[0].shape} def_out shape: {def_out.shape} y_seg shape: {y_seg.shape}")
                    # Val S09.delineation.skullstripped.nii.gz output 
                    # shape: torch.Size([1, 1, 64, 64, 64]) def_out shape: torch.Size([1, 1, 64, 64, 64]) 
                    # y_seg shape: torch.Size([1, 1, 64, 64, 64])
                    # print(f"Val {name} out x_seg: {torch.unique(x_seg)} def_out: {torch.unique(def_out)}")
