...
def register(model, epoch, inshape, logger, args):
    ...
    for p in pairlist:
        ...
        input_moving_seg = torch.from_numpy(src_seg).to(device).float().permute(0, 4, 1, 2, 3)
        input_fixed_seg = torch.from_numpy(trg_seg).to(device).float().permute(0, 4, 1, 2, 3)
        input_moving = F.interpolate(input_moving, size=inshape, mode='nearest') # 'nearest' 'area' 'trilinear') 
        input_fixed = F.interpolate(input_fixed, size=inshape, mode='nearest') # 'nearest' 'area' 'trilinear') )
        input_moving_seg = F.interpolate(input_moving_seg, size=inshape, mode='nearest') # 'nearest' 'area' 'trilinear') )
        input_fixed_seg = F.interpolate(input_fixed_seg, size=inshape, mode='nearest') # 'nearest' 'area' 'trilinear') )
        # print(f"register input_moving: {input_moving.shape} input_fixed: {input_fixed.shape} input_moving_seg: {input_moving_seg.shape}")
        ...
        dsc = ACDC_dice_val_VOI(moved_seg, fixed_seg_array)
        dsc_list.append(dsc)
        logger.info(f"Evaluation {p[2]} Dice: {dsc}")
...
def train(args, logger, device):
    ...
    imgshape = next(generator)[0][0].shape[1:-1]
    ...
    model.to(device)
    # model.train()
    logger.info(f"VoxelMorph image shape: {imgshape} reshape: {inshape} \nModel: {model}")
    # VoxelMorph image shape: (208, 256, 8) reshape: (256, 256, 16)
    ...
    for epoch in range(args.initial_epoch, args.epochs + 1):
        ...
        for step in range(args.steps_per_epoch):
            ...
            for n, loss_function in enumerate(losses):
                # print(f"n: {n} weights: {weights[n]} y_true: {y_true[n].shape} y_pred: {y_pred[n].shape}")
                # n: 0 weights: 1 y_true: torch.Size([1, 1, 160, 192, 224]) y_pred: torch.Size([1, 1, 160, 192, 224])
                # n: 1 weights: 0.01 y_true: torch.Size([1, 3, 160, 192, 224]) y_pred: torch.Size([1, 3, 80, 96, 112])
