...
def train(args, logger, device, checkpoint_dir):
    ...
    # print(f"*** Voxelmorph atlas: {args.atlas}")
    # *** Voxelmorph atlas: /mnt/lhz/Datasets/Learn2reg/LPBA40/fixed.nii.gz
    if args.atlas:
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        for step in range(args.steps_per_epoch):
            ...
            # inputs[0]: torch.Size([1, 1, 160, 192, 224]) inputs[1]: torch.Size([1, 1, 160, 192, 224]) n input: 2 
            # y_true[0]: torch.Size([1, 1, 160, 192, 224]) y_true[1]: torch.Size([1, 3, 160, 192, 224]) n y_true: 2             y_pred = model(*inputs)
...
if __name__ == "__main__":
    ...
    checkpoint_dir = os.path.join(model_dir, "VoxelMorph_LPBA_"+curr_time)
